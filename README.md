...
- git
- Кнопки «Finder» / «Terminal» работают на macOS, Linux и Windows. На Linux файловый
  менеджер открывается через `xdg-open`, а терминал — через первый найденный из
  konsole / gnome-terminal / kitty / alacritty / wezterm / foot / xfce4-terminal /
  tilix / x-terminal-emulator / xterm. Переопределить терминал: переменная
  `CLAUDECOCK_TERMINAL` (плейсхолдер `{dir}` — рабочая директория).
...
- **`merged` считается против *локального* `main`**, который в одноразовых
  ...
  шаг.
- Действия Finder/Terminal работают на macOS, Linux и Windows (см. «Требования»).
...
- git
- The "Finder" / "Terminal" buttons work on macOS, Linux and Windows. On Linux the
  file manager opens via `xdg-open` and the terminal via the first available of
  konsole / gnome-terminal / kitty / alacritty / wezterm / foot / xfce4-terminal /
  tilix / x-terminal-emulator / xterm. Override the terminal with the
  `CLAUDECOCK_TERMINAL` env var (`{dir}` placeholder = working directory).
...
| `CLAUDECOCK_TERMINAL` | *(auto-detect)* | Linux/other: command to open a terminal, e.g. `kitty -d {dir}`. `{dir}` is replaced with the target directory. |
...
- Finder/Terminal actions work on macOS, Linux and Windows (see Requirements).
